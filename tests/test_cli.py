...
from nginx_ssl_auto.cli import (
    check_command,
    create_parser,
    main,
    remove_command,
...
class TestCLI:
    ...
    def test_create_parser(self):
        ...
        assert (
            parser.description
            == "Automated SSL certificate management for Nginx using Let's Encrypt"
        )
    ...
    @patch("nginx_ssl_auto.cli.check_ssl_expiry")
    def test_check_command_success_active(self, mock_check):
        """Test successful check command with active certificate."""
        mock_check.return_value = {"success": True, "is_active": True}
    ...
    @patch("nginx_ssl_auto.cli.check_ssl_expiry")
    def test_check_command_success_expired(self, mock_check):
        """Test successful check command with expired certificate."""
        mock_check.return_value = {"success": True, "is_active": False}
    ...
    @patch("nginx_ssl_auto.cli.check_ssl_expiry")
    def test_check_command_failure(self, mock_check):
        """Test failed check command."""
        mock_check.return_value = {"success": False, "error": "Test error"}
    ...
    @patch("nginx_ssl_auto.cli.create_parser")
    def test_main_check_command(self, mock_create_parser):
        ...
        with patch("nginx_ssl_auto.cli.check_command") as mock_check:
            mock_check.return_value = 0
            result = main(["check", "example.com"])
            assert result == 0
